...
def call_daily_config(server_url, mac, token):
    ...
    if res.status_code != 200:
        ...
        res.close()
        raise OSError('daily-config HTTP %d' % code)
...
def call_daily_zip(server_url, mac, token, daily_zip_version, dest_path):
    ...
    if res.status_code != 200:
        code = res.status_code
        res.close()
        raise OSError('daily-zip HTTP %d' % code)
...
def call_refresh(server_url, mac, token, skip=False, battery=None):
    ...
    battery=<int 0-100> reports battery percentage; omitted when None.
    ...
    if res.status_code != 200:
        code = res.status_code
        res.close()
        raise OSError('refresh HTTP %d' % code)
    data = json.loads(res.text)
    res.close()
    return data
